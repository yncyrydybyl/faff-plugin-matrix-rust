...
- Encryption store lives at `~/.local/share/faff-plugin-matrix-rust/<id>/`, locked to `0700`. The directory contains the bot's Olm identity and Megolm session keys — don't delete it, and treat it like a private key. Set `[connection].store_passphrase` (or `store_passphrase_env`) for encryption at rest.
- Sends use `ignore_unverified_devices=true` — keys are shared with all devices in the room without manual verification.
- **Midnight rollover**: at 00:00 the Rust core auto-stops any unclosed session from the previous day, which triggers a `LogChanged` event and causes the watcher to emit a `stop` notification — even though the user took no action. This is currently accepted; suppression / a separate template is tracked in [#12](https://github.com/yncyrydybyl/faff-plugin-matrix-rust/issues/12).
- **Restart**: the watcher initialises its `prev` snapshot from the active session at startup, so a restart in the middle of a session emits no `start` notification, and a session that started **and** stopped while the watcher was down is missed entirely. Tracked in [#14](https://github.com/yncyrydybyl/faff-plugin-matrix-rust/issues/14).
